const { ActorQueryOperation, ActorQueryOperationTypedMediated } = require('@comunica/bus-query-operation');
...
class ActorQueryOperationMyReduced extends ActorQueryOperationTypedMediated {
  ...
  async runOperation(pattern, context) {
    ...
    logger.debug('Hello world!', { some: 'data', actor: this });
    ...
    const bindingsStream = output.bindingsStream.filter((binding) => {
      ...
      if (!cache[hash]) {
        ...
        // Clear a random cache entry if the cache becomes too big
        const currentCacheSize = Object.keys(cache).length;
        if (currentCacheSize > this.cacheSize) {
          const removeHash = Object.keys(cache)[Math.floor(Math.random() * currentCacheSize)];
          logger.debug('Cache eviction', { removeHash });
          delete cache[removeHash];
        }
